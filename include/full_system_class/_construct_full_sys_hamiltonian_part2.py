...
def _reverse_mat_diag_form(self):
    ...
    self.full_H._mat = self.full_H._diagonal_mat.copy()

    self.d1_H._mat = self.d1_H._diagonal_mat.copy()
    self.d2_H._mat = self.d2_H._diagonal_mat.copy()
...
def _construct_full_system_offdiag_coupling(self):
    ...
    for i in range(pd_dd_coupling_num):
        self.full_H._mat.append(self.pd_dd_offdiag_param[i])
        self.full_H._mat.append(self.pd_dd_offdiag_param[i])

    # coupling in detector 1
    intra_d1_coupling_num = len(self.d1_coupling_H._irow)
    construct_intra_d_coup(intra_d1_coupling_num, self.d1_coupling_dmat_index, self.detector1._d_Hamiltonian._mat, self.d1_H)

    # coupling in detector2
    intra_d2_coupling_num = len(self.d2_coupling_H._irow)
    construct_intra_d_coup(intra_d2_coupling_num, self.d2_coupling_dmat_index, self.detector2._d_Hamiltonian._mat, self.d2_H)
